...
int partition2(int *v, int low, int high)
{
...
void quickSort(int *v, int low, int high, int opt)
{
    int pivot_index;
    if (low < high)
    {
        if(opt == 1){
            pivot_index = partition(v, low, high);
        }
        if(opt == 2){
            pivot_index = partition2(v, low, high);

        }
        
        quickSort(v, low, pivot_index - 1, opt);
        quickSort(v, pivot_index + 1, high, opt);
    }
}
void quickSortStart(int *v, int lenght, int opt)
{
    struct timeval start, stop;
    gettimeofday(&start, NULL);
    srand(time(NULL));
    quickSort(v, 0, lenght - 1, opt);
